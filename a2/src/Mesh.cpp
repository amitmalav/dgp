...
Mesh::decimateQuadricEdgeCollapse()
{
  ...
  EdgeIterator edge_min = edgesBegin();
  for (EdgeIterator i = edgesBegin(); i != edgesEnd(); ++i)
  {
    if (i == edgesEnd())
    {
      return NULL;
    }
    if (i->getQuadricCollapseError() >= 0 && i->getQuadricCollapseError()<min_error)
    {
      edge_min = i;
      min_error = i->getQuadricCollapseError();
    }
  }
  
  // 2
  if (edge_min == edgesBegin() && min_error != edge_min->getQuadricCollapseError())
  {
    return NULL;
  }
  Vertex* v = collapseEdge(&(*(edge_min)));
  
  // 3
  for (MeshVertex::FaceIterator i = v->facesBegin(); i != v->facesEnd(); ++i)
  {
    if (i == v->facesEnd())
    {
      break;
  ...
  // 5
  for (MeshVertex::EdgeIterator i = v->edgesBegin(); i != v->edgesEnd(); ++i)
  {
    if (i == v->edgesEnd())
    {
      break;
    }
    Vertex* u = (*i)->getOtherEndpoint(v);
    u->updateQuadric();
    u->updateNormal();
    (*i)->updateQuadricCollapseError();
  }
